...
class ONLSTMCell(nn.Module):
    ...
    def forward(self, input, hidden, input_floor, forget_floor, transformed_input=None):
        ...
        return hy.view(-1, self.hidden_size), cy, (cingate, cforgetgate)
...
class ONLSTMStack(nn.Module):
    ...
    def __init__(
        ...
        self.layer_sizes = lyric_layer_sizes
        # lyric_onlstm
        self.cells = nn.ModuleList(
            [
                ONLSTMCell(lyric_layer_sizes[i], lyric_layer_sizes[i + 1], n_chunk, dropconnect=dropconnect)
                for i in range(len(lyric_layer_sizes) - 1)
            ]
        )
        ...
        self.tune_cells = nn.ModuleList(
            [
                tune_ONLSTMCell(tune_layer_sizes[i], tune_layer_sizes[i + 1], n_chunk, dropconnect=dropconnect)
                for i in range(len(tune_layer_sizes) - 1)
            ]
        )
        ...
        self.emb = nn.Embedding(11, n_chunk)

        self.dropoutw = dropoutw

        self.MLP = nn.Sequential(nn.Linear(tune_layer_sizes[-1] * 2, 4),
                                 nn.Softmax())
...
if __name__ == "__main__":
    lyric = torch.Tensor(128, 30, 768).cuda()
    tune = torch.Tensor(128, 30, 600).cuda()
    tree_posi = torch.randint(10, [128, 30]).cuda()
    lstm = ONLSTMStack([768, 384, 72],[600,384,72], n_chunk=12)
    lstm = lstm.cuda()
